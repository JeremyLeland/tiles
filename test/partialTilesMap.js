...
gameCanvas.pointerDown = ( m ) => {
  ...
  if ( wholeX && wholeY ) {
    ...
    const S_index = Math.floor( gridPoint[ 0 ] ) + gridPoint[ 1 ] * cols;
    ...
    const E_index = gridPoint[ 0 ] + Math.floor( gridPoint[ 1 ] ) * cols;
    const W_index = E_index - 1;

    console.log( W_index, E_index );
